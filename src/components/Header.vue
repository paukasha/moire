<template >
  <header class="header container">
    <div class="header__wrapper">
      ...
      <router-link :to="{name: 'MainPage', path: '/'}" class="header__logo" href="#">
      ...
      <router-link :to="{name: 'Basket', path:'/basket'}" class="header__cart"
                   aria-label="Корзина с товарами">
        <svg width="19" height="24">
          <use xlink:href="#icon-cart"></use>
        </svg>
        <span class="header__count" aria-label="Количество товаров">{{ productsQuantity }}</span>
      </router-link>
    </div>
  </header>
</template >
